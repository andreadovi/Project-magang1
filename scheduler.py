...
def try_schedule_on_mixer(mixer_name, mixer_schedule, deadline_idx,
    ...
    """
    Try to schedule all target_kg on a SINGLE mixer, filling slots
    consecutively backwards from deadline_idx.
    Returns list of assignments if successful, None otherwise.
    """
    remaining    = target_kg
    assignments  = []
    ...
    while remaining > 0:
        if search_idx < 0:
            return None  # ran out of history
        ...
        if last_grup is not None and last_grup != grup_produk:
            # Cleaning needed — skip this slot (cleaning will be inserted on commit)
            search_idx -= 1
            continue
